...
public class ExtendedRendererEntity extends PathfinderMob implements Animatable {
    ...
    private <E extends Animatable> PlayState predicateSpinHands(AnimationEvent<E> event) {
        if (event.getController().getCurrentAnimation() == null) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation(ANIM_NAME_SPIN_HANDS, LOOP));
        }
    ...
    private <E extends Animatable> PlayState predicateIdle(AnimationEvent<E> event) {
        if (event.getController().getCurrentAnimation() == null) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation(ANIM_NAME_IDLE, LOOP));
        }
        return PlayState.CONTINUE;
    }
    ...
    private <E extends Animatable> PlayState predicateBodyPose(AnimationEvent<E> event) {
        if (this.isTwoHandedAnimationRunning()) {
            ...
            event.getController().setAnimation(new AnimationBuilder().addAnimation(ANIM_NAME_SITTING, LOOP));
            return PlayState.CONTINUE;
        } else if (this.isCrouching()) {
            event.getController().setAnimation(new AnimationBuilder().addAnimation(ANIM_NAME_SNEAKING, LOOP));
            return PlayState.CONTINUE;
        }
        ...
        if (!handItemStack.isEmpty() && !this.isTwoHandedAnimationRunning()) {
            ...
            if (this.isBlocking()
                ...
                event.getController().setAnimation(new AnimationBuilder()
                        .addAnimation(leftHand ? ANIM_NAME_BLOCKING_LEFT : ANIM_NAME_BLOCKING_RIGHT, LOOP));
            } else {
    ...
    private <E extends Animatable> PlayState predicateTwoHandedPose(AnimationEvent<E> event) {
        if (this.isTwoHandedAnimationRunning()) {
            if (this.isSpellCasting()) {
                event.getController().setAnimation(new AnimationBuilder().addAnimation(ANIM_NAME_SPELLCASTING, LOOP));
                return PlayState.CONTINUE;
            } else {
    ...
    private <E extends Animatable> Optional<PlayState> performTwoHandedLogicPerHand(ItemStack itemStack,
        ...
        if (item.getUseAnimation(itemStack) == UseAnim.BOW || item.getUseAnimation(itemStack) == UseAnim.CROSSBOW) {
            // Firearm
            event.getController().setAnimation(new AnimationBuilder()
                    .addAnimation(leftHanded ? ANIM_NAME_FIREARM_POSE_LEFT : ANIM_NAME_FIREARM_POSE_RIGHT, LOOP));
            ...
            // Spear
            event.getController().setAnimation(new AnimationBuilder()
                    .addAnimation(leftHanded ? ANIM_NAME_SPEAR_POSE_LEFT : ANIM_NAME_SPEAR_POSE_RIGHT, LOOP));
            return Optional.of(PlayState.CONTINUE);
        }
    ...
    private <E extends Animatable> PlayState predicateTwoHandedSwing(AnimationEvent<E> event) {
        if (this.isTwoHandedAnimationRunning() && this.swinging) {
            ...
            if (this.getMainHandItem().getItem().getUseAnimation(this.getMainHandItem()) == UseAnim.SPEAR
                ...
                // Spear use animation
                event.getController().setAnimation(new AnimationBuilder().addAnimation(ANIM_NAME_SPEAR_SWING, PLAY_ONCE));
            }
